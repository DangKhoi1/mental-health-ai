...
import { sleepLogService } from '@/services/sleepLog';
import { toast } from 'sonner';

interface TrashedSleepLog {
    sleepLogId: string;
    sleepDate: string;
    deletedAt: string;
}

export default function SleepLogTrash() {
    const [items, setItems] = useState<TrashedSleepLog[]>([]);
    const [isLoading, setIsLoading] = useState(false);

    const fetchItems = async () => {
        setIsLoading(true);
        try {
            const res = await sleepLogService.getTrashed(1, 50);
            setItems((res?.data?.sleepLogs as unknown as TrashedSleepLog[]) || []);
        } catch {
    ...
    const handleRestore = async (id: string) => {
        try {
            ...
            if (res?.EC === 1) {
                ...
                toast.error(res?.EM || 'Khôi phục thất bại. Kiểm tra bản ghi trùng lặp.');
            }
        } catch {
        ...
        <div suppressHydrationWarning className="border rounded-xl overflow-hidden bg-card mt-6">
            {isLoading ? (
                ...
                <div className="divide-y divide-border">
                    {items.map((item) => (
                        <div key={item.sleepLogId} className="p-5 flex items-center justify-between hover:bg-muted/10 transition-colors group">
